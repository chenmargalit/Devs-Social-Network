...
router.get('/', (req, res) => {
  Post.find()
    ...
    .catch(err =>
      res.status(404).json({ nopostfound: 'No posts were found with that ID' })
    );
});
      ...
      Post.findById(req.params.id)
        .then(post => {
          ...
          if (post.user.toString() !== req.user.id) {
            return res
              .status(401)
              .json({ notauthorized: 'You can only delete your own posts' });
          }
    ...
    Post.findById(req.params.id)
      .then(post => {
        ...
        const removeIndex = post.comments
          ...
          .indexOf(req.params.comment_id); // I'm taking the index of the item._id that matches comment_id
